...
def read_csv(data_path, info_path, shuffle=False):
    D = pd.read_csv(data_path, header=None, on_bad_lines='skip')
    print(D.shape)
...
def read_csv(data_path, info_path, shuffle=False):
    ...
    # Read and prepare column information
    f_list, label_pos = read_info(info_path)
    f_df = pd.DataFrame(f_list)

    # Debug: Check the DataFrame shapes and contents
    print("D shape:", D.shape)
    print("f_df shape (expected columns):", f_df.shape)
    print("Sample f_df contents:", f_df.iloc[:, 0].head())

    # Assign columns if shapes match
    if D.shape[1] == f_df.shape[0]:
        D.columns = f_df.iloc[:, 0]
    else:
...
class DBEncoder:
    ...
    def split_data(self, X_df):
        ...
        if not continuous_data.empty:
            # Replace non-numeric entries with NaN and convert to float
